...
namespace sankaest\gii\generators\model;
 ...
 * @since 2.0
 */
class Generator extends \sankaest\gii\Generator
{
    ...
    public $tableName;
    public $modelClass;
    public $baseClass = 'sankaest\rebase\ActiveRecord';
    ...
    public $crudModelClass;
    public $crudSearchModelClass;
    public $crudControllerClass;
    public $crudViewPath;
    ...
    public function rules()
    {
        return array_merge(parent::rules(), [
            [['db', 'ns', 'tableName', 'modelClass', 'baseClass', 'queryNs', 'queryClass', 'queryBaseClass', 'moduleName'], 'filter', 'filter' => 'trim'],
            [['ns', 'queryNs', 'crudModelClass', 'crudSearchModelClass', 'crudControllerClass', 'crudViewPath'], 'filter', 'filter' => function ($value) { return trim($value, '\\'); }],
...
//    }
    ...
    public function afterValidate()
    {
        ...
        if($this->moduleName) {
            $this->ns = "common\\modules\\$this->moduleName\\models";
            $controllers = $views = ['common', 'modules',$this->moduleName ];
        }
        else {

            $controllers = $views = ['backend'];
            $this->ns = "common\\models";
        }

        $this->queryNs =  $this->ns.'\query';
        $parts = explode('\\', $this->ns);
        array_pop($parts);
        array_push($controllers,'controllers');
        array_push($views,'views');

        $this->crudModelClass = $this->ns.'\\'.$this->modelClass;
        $this->crudSearchModelClass = $this->ns.'\\search\\'.$this->modelClass.'Search';
        $this->crudControllerClass = implode('\\', $controllers).'\\'.$this->modelClass.'Controller';
        $this->crudViewPath = Yii::getAlias('@base').DIRECTORY_SEPARATOR.implode('/', $views).'/'.Inflector::camel2id(StringHelper::basename($this->modelClass));
    }
}