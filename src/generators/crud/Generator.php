...
namespace sankaest\gii\generators\crud;
...
class Generator extends \sankaest\gii\Generator
{
    ...
    public $viewPath;
    public $baseControllerClass = 'backend\controllers\VectorController';
    ...
    public function rules()
    {
        return array_merge(parent::rules(), [
            [['controllerClass', 'modelClass', 'searchModelClass', 'baseControllerClass', 'viewPath'], 'filter', 'filter' => 'trim'],
    ...
    public function generate()
    {
        ...
        ddd('$templatePath peab saama vastava template /view category');
...
//    ],
    ...
    public function generateActiveField($attribute)
    {
        ...
        // _type
        if (preg_match('/\d{_type}$/i', $column->name)) {
            return "\$form->field(\$model, '$attribute')
            ->modaForm('/.../create', Yii::t('frontend_backend', 'Create ...'))
            ->dropDownList([], ['prompt' => ''])";
        }

        if (preg_match('/\d{_id}$/i', $column->name)) {
            return "\$form->field(\$model, '$attribute')
            ->modaForm('/.../create', Yii::t('frontend_backend', 'Create ...'))
            ->select2SingleAjax(
                Url::to(['/ajax/models-list']),
                Yii::t('frontend_backend', 'Select an area ...'),
                \$model->omavalitsus->title ?? ''
            )";
        }

        if ($column->phpType === 'datetime') {
            return "\$form->field(\$model, '$attribute')->dateTimePickerControl()";
        }

        if ($column->phpType === 'date') {
            return "\$form->field(\$model, '$attribute')->datePickerControl()";
        }

        if ($column->phpType === 'boolean') {
            return "\$form->field(\$model, '$attribute')->checkboxX()";
        }
